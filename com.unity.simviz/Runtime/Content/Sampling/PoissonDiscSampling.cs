...
namespace UnityEngine.SimViz.Content.Sampling
{
    ...
    {
        public const int defaultSamplingResolution = 30;
        ...
        public static NativeList<float2> BuildSamples(
            ...
            float minimumRadius,
            uint seed = 12345,
            int samplingResolution = defaultSamplingResolution)
        {
            if (width < 0)
            ...
            var samples = Sample(width, height, minimumRadius, seed, samplingResolution, Allocator.TempJob);
            return samples;
        }
