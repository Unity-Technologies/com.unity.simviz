...
namespace UnityEngine.SimViz.Content.Sampling
{
    ...
    public static class GeometrySampling
    {
        ...
        public static NativeArray<PointSampleGlobal> BuildSamplesFromRoadOutlineWithExtensionDistance(
            Road road, float samplesPerMeter, float extensionDistance, LaneType stopAtLane)
        {
            var polygon = BuildSamplesFromRoadOutline(road, samplesPerMeter, stopAtLane);
        ...
        internal static NativeArray<PointSampleGlobal> BuildSamplesForLaneSectionSide(Road road, SamplingStateRoad state,
            Side side, int numLanesToSample = int.MaxValue)
        {
            ...
            var numLanes = math.min(numLanesToSample, sideLanes.Count());
            // samples for each lane are in contiguous blocks [s0l0, s1l0, s2l0, s0l1, s1l1, s1l2, s0l2...]
            var samples =
                new NativeArray<PointSampleGlobal>(totalSampleCount * numLanes, Allocator.Temp);
            for (var sampleIdx = 0; sampleIdx < sampleCount; sampleIdx++)
            {
                var sSampleAbsolute = sampleIdx / samplesPerMeter + laneSection.sRoad;
                SampleLanesOneSide(road, state, side, sSampleAbsolute, sampleIdx, ref samples, numLanesToSample);
            }

            if (sampleAtEnd)
            {
                var sEnd = laneSection.sRoad + laneLength;
                SampleLanesOneSide(road, state, side, sEnd, totalSampleCount - 1, ref samples, numLanesToSample);
            }
