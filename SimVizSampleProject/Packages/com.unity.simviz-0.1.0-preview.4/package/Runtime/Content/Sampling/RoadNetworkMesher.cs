...
namespace UnityEngine.SimViz.Content.Sampling
{
    ...
    public class RoadNetworkMesher : MonoBehaviour
    {
            ...
            GameObject goLaneSection)
        {
            ...
            var lanes = GeometrySampling.GetLaneSectionSideUpTo(laneSection, side, LaneType.None);
            var numLanes = lanes?.Count() ?? 0;

            if (numLanes == 0)
                return;

            var samples = GeometrySampling.BuildSamplesForLaneSectionSide(road, state, side, numLanes);
