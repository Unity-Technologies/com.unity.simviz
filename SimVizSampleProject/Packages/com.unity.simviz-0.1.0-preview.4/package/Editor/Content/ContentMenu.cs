...
namespace UnityEngine.SimViz.Content.Editor
{
    public class ContentMenu : ScriptableObject
    {
        ...
        public static RoadNetworkDescription GetSelectedRoadNetwork()
        {
            ...
            // If the selection is from the project window, load the selected RND asset
            // If something in the scene hierarchy is selected, try to find the RoadNetworkReference
            return AssetDatabase.Contains(obj)
                ? GetRoadNetworkFromAssetPath(obj)
                : GetRoadNetworkFromGameObject(obj as GameObject);
        }
